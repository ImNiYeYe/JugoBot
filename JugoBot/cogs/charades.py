...
####################################Your code
    ...
    async def charades(self,ctx):
        ...
        embmsg.set_thumbnail(url = ctx.author.avatar_url)
        ...
        playersList = await getmsg.reactions[0].users().flatten()
        playersList.remove(self.client.user)
        ...
        if len(playersList) != 1:
            ...
            try: #try to do these
                ...
                if msg.author != ctx.author: #if msg author is not the ctx.author(the person that started the game) then
                    ...
                    embmsg.set_thumbnail(url = msg.author.avatar_url)
                    embmsg.set_footer(text=f'charades')
                    await ctx.channel.send(embed=embmsg)
...
def setup(client):
    client.add_cog(Charades(client)) #Change <Template>into ur class name
