        ...
        #Otherwise there's a <minigame> argument, so display the relevant leaderboard (if there is one)
            ...
            if os.path.exists(minigamePath):
                ...
                if authorID in minigameDict:
                    msgEmbed.set_footer(text = f'You have played {minigameDict[authorID]["plays"]} games with {minigameDict[authorID]["wins"]}({"{:+}".format(minigameDict[authorID]["pointsWon"])}) wins and {minigameDict[authorID]["losses"]}({"{:+}".format(minigameDict[authorID]["pointsLost"])}) losses', icon_url = ctx.author.avatar_url)
                else:
                    msgEmbed.set_footer(text = f'You have not played {minigame}', icon_url = ctx.author.avatar_url)
                await ctx.send(embed = msgEmbed)
            else:
    ...
    async def all(self, ctx):
        ...
        if os.path.exists(allPath):
            ...
            if authorID in allDict:
                msgEmbed.set_footer(text = f'You have {allDict[authorID]} points', icon_url = ctx.author.avatar_url)
            else:
                msgEmbed.set_footer(text = "You haven't played any minigames", icon_url = ctx.author.avatar_url)
            await ctx.send(embed = msgEmbed)
        else:
